...
export class HUDManager {
  ...
  private readonly handleBitEvolve = () => {
    this.refreshBitStatus();
    this.surfaceThenFade(this.bitStatusText, 0.85, 4000);
  };
  private readonly handleShardCollected = () => {
    this.refreshShardStatus();
    this.surfaceThenFade(this.shardStatusText, 0.8, 4000);
  };
  ...
  /**
   * Surface a HUD chip, hold long enough to read, then dissolve — the
   * diegetic-UI register (docs/VISION.md §5 wound #3): status surfaces on
   * change, never persists. Under reduced motion the chip stays visible
   * instead of animating away (legibility wins over the chrome rule).
   */
  private surfaceThenFade(
  ...
  private createCompanionStatus(scene: Phaser.Scene): void {
    ...
    this.shardStatusText = scene.add
      .text(x, y + 32, "", {
        fontSize: "8px",
        fontFamily: FONTS.RETRO,
        color: COLOR_HEX.TEXT_LIGHT,
    ...
    this.refreshBitStatus();
    this.refreshShardStatus();
    // Greet, then get out of the way — same register as the controls strip.
    // The chips resurface (then re-fade) on BIT_EVOLVE / SHARD_COLLECTED.
    this.surfaceThenFade(this.bitStatusText, 0.85, 7000);
    this.surfaceThenFade(this.shardStatusText, 0.8, 7000);
  }
  ...
  private createControlsStrip(scene: Phaser.Scene): void {
    const { width, height } = scene.cameras.main;
    this.controlsStripText = scene.add
      .text(
        width / 2,
        height - 12,
  ...
  private readonly handlePuzzleComplete = () => {
    ...
    if (this.encountersText.active && this.encountersText.text) {
      this.surfaceThenFade(this.encountersText, 0.8, 4000);
    }
  };
  ...
  private refreshEncountersText(): void {
    ...
    const progress = progressionSystem.getRegionEncounterProgress(
      this.activeRegionId,
    );
    if (!progress) {
    ...
    for (let i = 0; i < progress.total - 1; i++) {
      ledger +=
        i < Math.min(progress.completed, progress.total - 1) ? "[x]" : "[ ]";
    }
  ...
  private refreshBitStatus(): void {
    ...
    this.bitStatusText.setText(
      `BIT ${BIT_STAGE_GLYPH[stage]} ${BIT_STAGE_LABEL[stage]}`,
    );
  }
  ...
  showRegionName(name: string): void {
    ...
    const label = presentation
      ? `${progress.label} ${presentation.shortTitle}`
      : name;
  ...
  showRegionCard(name: string, subtitle: string): void {
    ...
    const presentation = getRegionPresentationByTitle(name);
    const progress = getRegionProgress(presentation?.id);
    const persistentLabel = presentation
      ? `${progress.label} ${presentation.shortTitle}`
      : name;
    ...
    const subText = this.scene.add
      .text(width / 2, cardY + 48, subtitle, {
        fontSize: "10px",
        fontFamily: FONTS.RETRO,
    ...
    const actLabel = presentation
      ...
      : "ACT ?/9";
    const actText = this.scene.add
      .text(width / 2, cardY + 76, actLabel, {
        fontSize: "8px",
        fontFamily: FONTS.RETRO,
    ...
    const hintText = this.scene.add
      .text(
        width / 2,
        cardY + 106,
        "SPACE ACT  |  C CODEX  |  M MUTE  |  ESC PAUSE",
        {
          fontSize: "7px",
          fontFamily: FONTS.RETRO,
          color: COLOR_HEX.TEXT_MUTED,
    ...
    const rail = this.createJourneyRail(
      width / 2,
      cardY + cardH - 12,
      progress.current,
      accent,
    ...
    this.cardTween = this.scene.tweens.add({
      ...
      yoyo: true,
      onComplete: () => {
        // Surface the small top-left badge as the card hands off, then let
        // it dissolve — nothing in the HUD persists (docs/VISION.md §5).
        this.surfaceThenFade(this.regionText, 0.6, 5000);
        if (this.encountersText.text) {
          this.surfaceThenFade(this.encountersText, 0.6, 5000);
        }
        this.destroyCard();
      },
    });
  ...
  private createJourneyRail(
    ...
    const startX = Math.round(
      centerX - ((REGION_PRESENTATION.length - 1) * dotSpacing) / 2,
    );
